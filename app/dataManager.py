...
def load_data(file_path):
    data = pd.read_csv(file_path, sep='\s+')#delim_whitespace=True)
    return data
...
class AnaSample:
    ...
    def set_SK_true_data_as_data_hist(self):
        ...
        for local_bin_idx, global_bin_idx in enumerate(self.bin_indices):
            ...
            # this is a trick to avoid a warning. Pandas doesn't like to use int directly on a Series (e.g. int(df.iloc[global_bin_idx]))
            self.data_hist.SetBinContent(int(local_bin_idx+1), int(df.iloc[global_bin_idx].to_numpy()[0]))
